...
slug: mastering-sbom-generation-with-yocto
---
...
Since Yocto builds everything from source, it can provide a complete SBOM for all components it compiles. Because Yocto has a catalog of all the packages it can build, including the corresponding licensing, it is able to generate high-quality SBOMs out of the box. Other platforms are also adopting this approach; for example, the Raspberry Pi Foundation recently introduced [built-in SBOM generation with `rpi-image-gen`](/2025/04/17/unpacking-raspberry-pi-s-built-in-sbom-magic/).
...
Moreover, since sbomify supports [SBOM hierarchy natively](/features/sbom-hierarchy/), there is no need for `index.json`. Moreover, since we've mapped each package to a component, once we release an update to a package, we can track this and have a clear audit trail of the changes.
