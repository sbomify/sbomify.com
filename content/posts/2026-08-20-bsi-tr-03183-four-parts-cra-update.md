...
faq:
  ...
  - question: "What is new in BSI TR-03183-1 version 1.0.0?"
    answer: "BSI published Part 1 (General requirements) in version 1.0.0 on 31 July 2026 and announced it on 5 August 2026. It introduces Adaptable Risk-based Controls (ARC), where each control is paired with risk scenarios describing affected assets, impact, and environment parameters, and it ships an initial control set in machine-readable OSCAL format on GitHub. It is explicitly non-binding and gives no presumption of conformity."
  ...
  - question: "Are CRA harmonised standards available yet?"
    answer: "No. The horizontal EN 40000 series and the vertical EN 50770 series are still in development, and no CRA harmonised standard has been cited in the Official Journal. Until one is, the Article 27 presumption of conformity is unavailable. A draft Commission amendment in July 2026 pushed the Type A and Type B delivery deadlines from 30 August 2026 to 31 October 2026, and Type C from 30 October to 31 December 2026."
  - question: "What do I need to do before 11 September 2026?"
    answer: "Register on the ENISA Single Reporting Platform through EU Login with a Primary and Secondary representative, because there is no submission API and no way to register retroactively inside a 24-hour window. Have SBOMs for every shipped version so you can answer which products contain an exploited component, and publish a security.txt and coordinated vulnerability disclosure policy so reports reach you in the first place."
date: 2026-08-20
...
The interesting mechanism is what BSI calls **Adaptable Risk-based Controls (ARC)**. Rather than handing you a flat checklist, every control comes attached to one or more risk scenarios, each describing the affected assets and impact plus environment parameters: access restriction, interface restriction, user capabilities. You categorise your product's assets by confidentiality, integrity and availability impact, describe the environment each component operates in, and select controls where both the asset impact and the environment match. It is a way of making "appropriate to the risk" concrete without pretending every product is the same.

The part that should interest anyone building compliance tooling: BSI ships the control catalogue in **OSCAL**, NIST's Open Security Controls Assessment Language, published at [github.com/tr-03183/tr-03183-1](https://github.com/tr-03183/tr-03183-1) with access details available on request. Machine-readable controls mean filtering by use case, building custom catalogues, and generating structured assessment evidence rather than another spreadsheet. It is the same instinct that drives machine-readable SBOMs, applied one level up.
...
**The Commission published its first CRA application guidance on 27 July 2026** (C(2026) 5252). It runs to roughly 80 pages with 67 practical examples, flowcharts and use cases, and works through the questions manufacturers actually ask: what counts as a product with digital elements, when free and open-source software falls in scope, what makes a modification substantial, how support periods work, and how reporting obligations apply. It is non-binding, and only the Court of Justice can authoritatively interpret the CRA, but it will shape how market surveillance authorities and notified bodies read the regulation.

One clarification from that guidance is directly relevant to anyone triaging component vulnerabilities: a vulnerability in a third-party component is a reportable actively exploited vulnerability in **your** product only where the vulnerable code is actually reachable and exploitable there. That is a VEX-shaped question, and answering it under time pressure presupposes an SBOM you trust.

**ENISA published step-by-step Single Reporting Platform instructions on 31 July 2026.** Three details deserve attention:

1. **Registration happens through EU Login**, with a nominated Primary and Secondary representative, and the Secondary receives a time-limited invitation link. You can and should do this before the platform goes live. Nobody registers an organisation inside a 24-hour early-warning window.
2. **There is no reporting API at launch.** You can automate your internal workflow right up to submission, but a human types the final form into a browser. That makes the quality of your internal triage data the rate-limiting factor, not your integration work.
...
1. **Register on the ENISA Single Reporting Platform.** EU Login accounts, a Primary and a Secondary representative, done now. There is no API, so make sure the people who will actually submit have working access.
